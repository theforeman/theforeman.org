...
## 2.3 Installing hammer_cli_foreman_openscap

It is possible to install a Hammer plugin containing commands specific to openscap with:
...
# 3. Using datastream files for scanning
...
# 4. Using OVAL files for scanning

The OVAL feature in foreman_openscap is a tech preview. To show the feature menu items in UI, go to Administer -> Settings, General tab and make sure the 'Show Experimental Labs' is turned on. 

## 4.1 Creating OVAL content
* Go to Lab Features -> OVAL Contents
* Click 'Create OVAL content' button
* Fill in the required fields. It is possible to upload a file with OVAL content or specify a file which is available via HTTP(S)
* Submit

## 4.2 Crating OVAL policy
* Go to Lab Features -> OVAL Policies
* Click 'Create OVAL policy' button
* Fill in the required fields
* Submit

## 4.3 Scanning hosts with OVAL policies

For now, it is possible to assign policies to hosts only via hostgroups. The same mechanism of client configuration is used as for scans using datastream files. The only option for OVAL policy deployment is Ansible for now, therefore hostgroups are required to have theforeman.foreman_scap_client Ansible role. Make sure hostgroup also have OpenSCAP proxy before assigning OVAL policy.

## 4.4 Check for scan prerequisities
(Not yet available, fixed in #33653)

There are several things that are required for successfull configuration of hosts for scans. Some need to be performed manually by users (importing theforeman.foreman_scap_client Ansible role) while others are done silently behind the scenes to ease the burden on a user. There is always a possibility of a misconfiguration, so foreman_openscap provides a convenient way to check if things are set up correctly.

* Go to Lab Features -> OVAL Policies
* Click 'Check OVAL Setup' button
* Click 'Check' button

You will be presented with a list of performed checks, their results and possible steps for remediating any check failures.
...
## 6.6 Slow queries due to many message records in database
...
## 6.7 Cleaning up reports without OpenSCAP proxy
