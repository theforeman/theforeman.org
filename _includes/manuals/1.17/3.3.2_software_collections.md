...
A "tfm" (The Foreman) collection is provided by the Foreman project which ships all dependencies for the main Foreman application, and depends on the "tfm-ror51" and "rh-ruby24" collections which provide newer versions of Ruby and Ruby on Rails.  Packages that are part of these collections will have "tfm-", "tfm-ror51-" and "rh-ruby24-" prefixes, allowing them to be easily identified, and will install entirely underneath `/opt/theforeman` and `/opt/rh`.
