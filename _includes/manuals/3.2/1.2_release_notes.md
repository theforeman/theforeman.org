...
#### Host reports (preview)

Configuration Reports are now moving to Host Reports, completely new format of storing configuration reports in a more efficient way. Our measurements show that reporting is now 20 times faster while taking 50% less storage in PostgreSQL database. Host Reports are stored in JSON format and currently supported configuration reports are Puppet and Ansible. Each report has three counters which represent amount of resources (or tasks) that were changed, unchanged or failed. On top of that, each report can have report "keywords" which are searchable. To filter out puppet reports witch failure in particular resource, use "keyword = PuppetFailed:File[/path/that/does/not/exist/causes/failures]". Searching is intivutive as the search box supports query completion.

Host Reports are implemented in separate plugins called Foreman Host Reports and Smart Proxy Reports. See upgrade warnings below for migration instructions.
...
#### Migrating to Host Reports (preview)
...
For step-by-step instructions see the [Migrating Reports to the new format in Foreman 3.2](https://community.theforeman.org/t/migrating-reports-to-the-new-format-in-foreman-3-2/25846) community post.
...
The Configuration Reports and the API endpoints are deprecated and will be removed in future Foreman versions. Follow the Migrating to Host Reports in the Upgrade warning section to migrate to the new format and update Smart Proxy configuration for Puppet and Ansible to send reports to the new endpoints.
