...
* **Smart matchers returning incorrect values:** smart class matchers have
  been found to be returning incorrect values when overriding two host groups
  from the same hierarchy ([#8551](http://projects.theforeman.org/issues/8551))
* **$::interfaces fact is overridden:** 1.7.0 adds 'interfaces' to the ENC
  YAML output, which overrides the $::interfaces fact.  If any module or
  manifest uses this fact, it may raise an error as the data is different
  ([#8546](http://projects.theforeman.org/issues/8546)).  1.7.1 will rename
  the parameter from the ENC.
