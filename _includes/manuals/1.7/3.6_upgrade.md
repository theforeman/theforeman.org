...
* **API error when using some parameters:** some API requests will fail
  with a "is not allowed as nested parameter" message ([#7989](http://projects.theforeman.org/issues/7989))
* **Secure console connection is denied:** when using a secure websockets
  connection for a host console, security settings may deny it ([#8091](http://projects.theforeman.org/issues/8091))
* **Additional audit entries setting grub pass:** as hosts are updated,
  an additional audit entry setting grub pass will be seen ([#7655](http://projects.theforeman.org/issues/7655))
* **Password not shown after failed install:** when an error happens during
  installation, the admin password isn't shown ([#8008](http://projects.theforeman.org/issues/8008))
