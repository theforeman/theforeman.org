...
{% highlight bash %}
cp example.pem /etc/pki/tls/certs/
ln -s example.pem /etc/pki/tls/certs/$(openssl x509 -noout -hash -in /etc/pki/tls/certs/example.pem).0
{% endhighlight %}

On Debian or Ubuntu:

{% highlight bash %}
cp example.pem /usr/local/share/ca-certificates/
