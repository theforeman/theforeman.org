...
#### Manual installation of Smart Proxy

To integrate Foreman Proxy with existing services, manual configuration needs
to be done. In that case install the `foreman-proxy` package and proceed with
editing `/etc/foreman-proxy` configuration files. The service name to restart
is named `foreman-proxy`.

We don't provide details about manual configuration, so the recommended way is
to install Proxy on another host via our installer and then compare the
configuration values with the target host.
