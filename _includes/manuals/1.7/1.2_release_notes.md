...
* Pin wirb for Ruby 1.9 compatibility ([#9941](http://projects.theforeman.org/issues/9941))
* Fix missing HOME environment variable error with apipie-bindings ([#9990](http://projects.theforeman.org/issues/9990))

#### Security
* Restrict users to assigned organizations and locations when associated to more than one ([CVE-2015-1844](/security.html#2015-1844), [#9947](http://projects.theforeman.org/issues/9947))
* Refactor restriction of orgs/locations in hosts to also fix Foreman Discovery ([#10025](http://projects.theforeman.org/issues/10025))

*A full list of changes in 1.7.5 is available via [Redmine](http://projects.theforeman.org/rb/release/40)*

### Release notes for 1.7.4

Foreman 1.7.4 is a security and bug fix update for all components.

Any Foreman installation that has LDAP authentication over SSL configured should be upgraded to fix the security vulnerability.  The LDAP server certificate authority will then need to be trusted before or after upgrading - see [section 4.1.1](/manuals/{{page.version}}/index.html#4.1.1LDAPAuthentication) for details.

#### Installer and packaging
* Pin gettext_i18n_rails_js for compatibility ([#9851](http://projects.theforeman.org/issues/9851))

#### Puppet and Puppet integration
* Fix handling of YAML and JSON matchers when generating ENC output ([#9673](http://projects.theforeman.org/issues/9673))
* Fix handling of YAML and JSON default values when generating ENC output ([#9787](http://projects.theforeman.org/issues/9787))
* Fix missing VLAN ID in foreman_interfaces ENC output ([#8779](http://projects.theforeman.org/issues/8779))

#### Security
* Verify LDAP server SSL certificate against trusted store ([CVE-2015-1816](/security.html#2015-1816), [#9858](http://projects.theforeman.org/issues/9858))

#### Web Interface
* Fix broken redirect after deleting reports ([#9469](http://projects.theforeman.org/issues/9469))

#### Various fixes and features
* Fix disabling of user email notifications ([#9795](http://projects.theforeman.org/issues/9795))

*A full list of changes in 1.7.4 is available via [Redmine](http://projects.theforeman.org/rb/release/36)*
...
#### API
* Fix compute resource URL being marked as required for EC2 ([#5812](http://projects.theforeman.org/issues/5812))

#### Compute resources and Hosts creation
* Fix `$version` in installation media to work when missing minor version number, i.e. CentOS 7 ([#6884](http://projects.theforeman.org/issues/6884))
* Fix inheritance of root passwords from settings and through host groups ([#8739](http://projects.theforeman.org/issues/8739))

#### Proxy and Services
* Fix access to template proxying module from non-trusted hosts ([#9102](http://projects.theforeman.org/issues/9102))

#### Installer and packaging
* Add gssapi Debian package for Kerberos support in Smart Proxy ([#9156](http://projects.theforeman.org/issues/9156))

#### Puppet and Puppet integration
* Fix validation of array and hash smart class parameters containing ERB ([#8052](http://projects.theforeman.org/issues/8052))
* Fix logging of error messages when rendering ENC output ([#9231](http://projects.theforeman.org/issues/9231))

#### Various fixes and features
* Fix database performance regression in report expiry rake task ([#8565](http://projects.theforeman.org/issues/8565))
* Fix database error when loading reports as non-admin user with filter ([#8817](http://projects.theforeman.org/issues/8817))
* Fix copying of config groups when cloning host groups ([#9274](http://projects.theforeman.org/issues/9274))
* Fix email notification error when host has no owner ([#9118](http://projects.theforeman.org/issues/9118))
* Fix foreman-config script to update settings correctly ([#9193](http://projects.theforeman.org/issues/9193))
* Fix MAC address generators in tests ([#9044](http://projects.theforeman.org/issues/9044))

*A full list of changes in 1.7.3 is available via [Redmine](http://projects.theforeman.org/rb/release/32)*
