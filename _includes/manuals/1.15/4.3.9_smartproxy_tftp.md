...
At the moment, the proxy is not able to fetch boot files using NFS. As a workaround, expose your installation medium (or use a public mirror) over http/ftp to configure one machine with the required boot files. Alternatively, you can still use NFS, but manual steps are required to make boot files available to your proxy:
...
To implement fully automatic installations from NFS, it would require to teach proxy how to mount NFS. We track it as #19045
