[The Foreman installer](/manuals/{{page.version}}/index.html#3.2ForemanInstaller) uses OpenVox **(8 or later required)** to install Foreman. This guide assumes that you have a newly installed operating system, on which the installer will setup Foreman, a OpenVox server, and the [Smart Proxy](/manuals/{{page.version}}/index.html#4.3SmartProxies) by default. It's **not advisable** to follow the steps below on an existing system, since the installer will affect the configuration of several components.
...
<select onChange="update_quickstart_os(this);">
  ...
  <option value="debian13">Debian 13 (Trixie)</option>
  <option value="ubuntu2204">Ubuntu 22.04 (Jammy)</option>
  <option value="ubuntu2404">Ubuntu 24.04 (Noble)</option>
</select>
...
<div class="quickstart_os quickstart_os_el9">
  <p>
    Enable OpenVox's 8.x repository:
  </p>
...
cd /tmp && wget https://apt.voxpupuli.org/openvox8-release-debian12.deb
sudo apt-get install /tmp/openvox8-release-debian12.deb
{% endhighlight %}
...
<div class="quickstart_os quickstart_os_debian13">
  <p>
    Enable OpenVox's 8.x repository:
  </p>
...
sudo apt-get install /tmp/openvox8-release-debian13.deb
{% endhighlight %}
...
{% highlight bash %}
sudo wget https://deb.theforeman.org/foreman.asc -O /etc/apt/trusted.gpg.d/foreman.asc
echo "deb http://deb.theforeman.org/ trixie {{page.version}}" | sudo tee /etc/apt/sources.list.d/foreman.list
echo "deb http://deb.theforeman.org/ plugins {{page.version}}" | sudo tee -a /etc/apt/sources.list.d/foreman.list
{% endhighlight %}
</div>
...
<div class="quickstart_os quickstart_os_ubuntu2404">
  <p>
    Enable OpenVox's 8.x repository:
  </p>
...
{% highlight bash %}
sudo apt-get -y install ca-certificates
cd /tmp && wget https://apt.voxpupuli.org/openvox8-release-ubuntu24.04.deb
sudo apt-get install /tmp/openvox8-release-ubuntu24.04.deb
{% endhighlight %}
...
{% highlight bash %}
sudo wget https://deb.theforeman.org/foreman.asc -O /etc/apt/trusted.gpg.d/foreman.asc
echo "deb http://deb.theforeman.org/ noble {{page.version}}" | sudo tee /etc/apt/sources.list.d/foreman.list
echo "deb http://deb.theforeman.org/ plugins {{page.version}}" | sudo tee -a /etc/apt/sources.list.d/foreman.list
{% endhighlight %}
</div>
...
<div class="quickstart_os quickstart_os_debian12 quickstart_os_debian13 quickstart_os_ubuntu2204 quickstart_os_ubuntu2404">
{% highlight bash %}
...
<div class="quickstart_os quickstart_os_debian12 quickstart_os_debian13 quickstart_os_ubuntu2204 quickstart_os_ubuntu2404 alert alert-info">
...
<div class="quickstart_os quickstart_os_none quickstart_os_debian12 quickstart_os_debian13 quickstart_os_ubuntu2204 quickstart_os_ubuntu2404 quickstart_os_el9">
{% highlight bash %}
