...
Optionally, configuration files should be reviewed. A good tool to find
`.rpmnew` and `.rpmsave` files is `rpmconf`, keep in mind that some
configuration files are managed by puppet therefore all changes made by the
tool will be overwritten with the next execution of the `foreman-installer`:
