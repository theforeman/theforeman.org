...
<div class="quickstart_os quickstart_os_rhel6 quickstart_os_el6 quickstart_os_rhel7 quickstart_os_el7 alert alert-warning">
The current Foreman installer isn't compatible with the new Puppet Server 2.6.0 release, so if using the Puppet 4.x (PC1) repos, please run <code>yum install puppetserver-2.5.0</code> before continuing.
</div>

<div class="quickstart_os quickstart_os_debian8 quickstart_os_ubuntu1404 quickstart_os_ubuntu1604 alert alert-warning">
The current Foreman installer isn't compatible with the new Puppet Server 2.6.0 release, so if using the Puppet 4.x (PC1) repos, please run <code>apt-get install puppetserver=2.5.0-1puppetlabs1</code> before continuing.
</div>
