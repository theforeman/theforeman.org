...
Since Foreman integrates with Puppet heavily, it is recommended to use the Puppet Certificate Authority (CA) to secure proxy access.  See the [Securing Communications with SSL](#5.4SecuringCommunicationswithSSL) section for more advanced installations (multiple or internal CAs).
...
Please note, the smart proxy uses the OpenSSL suite naming scheme. For more information on suite names please see [the OpenSSL docs](https://docs.openssl.org/master/man1/openssl-ciphers/#cipher-suite-names).
