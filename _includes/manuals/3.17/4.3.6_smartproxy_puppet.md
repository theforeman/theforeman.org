...
The Puppetserver has to permit these API queries. The [HOCON-formatted auth.conf style](https://help.puppet.com/core/current/Content/PuppetCore/server/config_file_auth.htm) is at /etc/puppetlabs/puppetserver/conf.d/auth.conf and requires these rules:
