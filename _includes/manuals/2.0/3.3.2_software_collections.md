...
A "tfm" (The Foreman) collection is provided by the Foreman project which ships all dependencies for the main Foreman application, and depends on the "tfm-ror52" and "rh-ruby25" collections which provide newer versions of Ruby and Ruby on Rails.  Packages that are part of these collections will have "tfm-", "tfm-ror52-" and "rh-ruby25-" prefixes, allowing them to be easily identified, and will install entirely underneath `/opt/theforeman` and `/opt/rh`.
