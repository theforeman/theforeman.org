...
New installer script based on project
[kafo](https://github.com/theforeman/kafo) is available. Refer to the
installation section of the manual to see how it works.

#### Smart-proxy auto-registration

In the default setup, smart-proxy is automatically registered using Foreman API
(via OAuth) during installation. You can change that behavior with
...
`rubygem-hammer_cli_foreman` first and:
...
clicking on Match VM's to Hosts at the Compute Resource Level, or the Manage
button, if available at the compute resource VM. There is logic to determine
...
Installation on RHEL6 and clones is leveraging SCL (Software Collections)
...
logging, documentation and smart classes support.
...
change to the latest version, which can be found in our git:

- [external_node.rb.erb](https://github.com/theforeman/puppet-foreman/tree/master/templates/external_node.rb.erb)
- [foreman-report.rb.erb](https://github.com/theforeman/puppet-foreman/tree/master/templates/foreman-report.rb.erb)
...
settings.yaml Foreman automatically updates database. See
...
There is new command `foreman-rake` which must be used instead `rake` for doing
ADMINISTRative tasks. It is a wrapper around `rake` that changes to the correct
directory and on SCL-enabled systems it does use `scl` command.

More about software collections is available in the [Software
Collections](manuals/{{page.version}}/index.html[#3](http://projects.theforeman.org/issues/3).3.2SoftwareCollections)
manual section.
