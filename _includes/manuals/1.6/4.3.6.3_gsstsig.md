...
Test the Kerberos login with that user on the Smart Proxy using the command kinit:
...
Now create the keytab file with `ktutil`:
    ...
    Password for foremanproxy@EXAMPLE.COM:
...
Now update the configuration in /etc/foreman-proxy/settings.d/dns.yml for the smart proxy:
...
Update the proxy configuration file (`/etc/foreman-proxy/settings.yml`) with the following settings:
