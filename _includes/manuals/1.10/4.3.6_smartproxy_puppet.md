...
Here is a quick howto to enable puppetssh on your server.
...
sed -i 's!^#\?\(:puppetssh_user:\).*$!\1 root!' \
  /etc/foreman-proxy/settings.d/puppet.yml
sed -i 's!^#\?\(:puppetssh_keyfile:\).*$!\1 /etc/foreman-proxy/id_rs!' \
  /etc/foreman-proxy/settings.d/puppet.yml
</pre>

We also need to actually generate this new SSH key:
<pre>
