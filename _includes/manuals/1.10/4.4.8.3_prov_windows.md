Provisioning Windows is a two step process. The first step, creating Installation Media, is not discussed here but can be found in the [Wiki](http://projects.theforeman.org/projects/foreman/wiki/Tutorials). The second step, setting up templates, is discussed in the following section.
...
If you already have Windows hosts and with Puppet installed, the correct OS and architecture has been auto created already.
...
Head to _Hosts -> Provisioning Templates_ and edit the templates starting with `WAIK` to your needs. Make sure to get the latest version of the WAIK templates from the [community templates project](https://github.com/theforeman/community-templates)
...
__Note__ You can find more info about [Foreman Template Writing in the Wiki](http://projects.theforeman.org/projects/foreman/wiki/TemplateWriting)
...
_Example_: Imagine host `example` in domain `ad.corp.com` and in hostgroup `servers/windows/databases`.
...
- `domainJoinAccountPasswd`: Pa55w@rd - Password for the domain Admin account
