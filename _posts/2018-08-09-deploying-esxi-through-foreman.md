...
* **Note**: When ESXi is deployed through Foreman, getting the host to perform a local boot from PXE can be tricky. In this article we will use foreman-hooks to work around issues that can crop up if the first boot entry of a host is PXE. If this is not the case in your environment, you may skip Step 3.
  ...
  * **1.3.1:** Create the PXELinux template
    ...
    * Template contents:
    ...
    <p float='left' align='left'>
      ...
      Note the **KERNEL** and **APPEND** lines. These will be useful when we actually mount the ISO image.
...
The ESXi PXELinux mboot.c32 module only supports syslinux bootloader (pxelinux.0) version 3.86. You may replace the newer pxelinux.0 file that comes with the default Foreman installation with pxelinux.0 v3.86 but then you might face issues with newer OSes. So we will use a different method.

**2.1: Prepare the bootloader**  
On the Foreman server, extract syslinux-3.86.tar.gz. Copy `syslinux-3.86/core/pxelinux.0` to the tftp root directory (typically `/var/lib/tftpboot`) and rename it to `pxelinux-3.86.0`
    ...
    def all_loaders_map(precision = 'x64')
      {
        ...
        <b>"ESXi PXELinux BIOS" => "pxelinux-3.86.0",</b> # Add this line
        ...
    </pre></code>
...
Copy the boot.cfg file from the mountpoint to tftproot and edit it to add a prefix to the ISO mountpoint. Make sure to use the same filename as the one specified in the **APPEND** line of the PXELinux template.
  ...
  Note: Instead of mounting the ISO, you can also extract the ISO contents to the directory and edit the boot.cfg directly inside that directory. Remember to change the PXELinux template accordingly. I chose this because this approach is a tad cleaner.  
...
Now once installation is complete and the host reboots, it will try to PXEBoot and **should** be presented with the default PXE menu.
<p float='left' align='center'>
  <img src="/static/images/blog_images/2018-08-16-deploying-esxi-through-foreman/localboot.png" width="400" />
</p>
Upon timeout, the host should attempt a Localboot and boot to ESXi.

However, since the host will boot into the pxelinux.0 v3.86 bootloader, without the supporting files of the same version (menu.c32 etc) the host will not be presented with this PXE menu and will never boot into ESXi.

To solve this we have to manually change the host PXE Loader to **PXELinux BIOS** or **None** in Foreman.  
This can be quite tedious when we are deploying at scale. Here is where Foreman-Hooks comes to the rescue.
...
### 3: Using Foreman hooks to solve host not able to perform local boot from PXE
---

Once the host informs Foreman that build is completed, we will run a script that checks if the host's OS is ESXi and then changes the PXELoader to None. For this we need to hook into the before_provision event.
...
# Script to check if OS Name starts with ESXi and change PXE Loader to None
...
if [[ $system_operatingsystem_name == ESXi* ]]; then
    echo "ESXi OS detected. Changing PXE Loader to None"
...
With the hook script enabled, when a host finishes OS installation and sends a build complete call to Foreman, its PXE Loader will be automatically changed to "None".
So next time when it boots, it will load Foreman's default pxelinux.0 from the DHCP server and be presented with the default PXE menu with Localboot at the top.

### **4:** ESXi installation in UEFI mode

---

* **4.1: Prepare the bootloader**  
...
* **4.2: Create the bootloader entry in Foreman**  
    ...
    def all_loaders_map(precision = 'x64')
      {
        ...
        "PXELinux BIOS" => "pxelinux.0",
        "PXELinux UEFI" => "pxelinux.efi",
        "ESXi PXELinux BIOS" => "pxelinux-3.86.0",
        <b>"ESXi mboot UEFI" => "mboot.efi",</b> # Add this line
        ...
    </pre></code>

  * Restart httpd/apache2 to register the changes

* **4.3:** Once the host loads mboot.efi, it will look for a boot.cfg file under tftproot/01-{MAC address of host}. Once it is located, the installer loads up and performs the installation. So each time we need to deploy ESXi in UEFI mode we need to create this directory and copy over the boot.cfg file.  

  Evidently this is something foreman-hook can help us automate. Once we set the host to build mode, this directory along with the boot.cfg file should be created automatically.  For this we need to hook into the after_build event.
  ...
  echo "$(date): received ${event} on ${object}"
  if [[ $system_operatingsystem_name == ESXi* && $system_pxe_loader == ESXi mboot UEFI ]]; then
...
* **4.4:** Now we are ready to deploy the host. Edit the host entry in Foreman and set the following:

  Operating System : ESXi 6.7 Build 8169922  
  Media : ESXi Dummy  
  Partition table: Kickstart default  
  PXE Loader : ESXi mboot UEFI  
  <p float='left' align='left'>
    <img src="/static/images/blog_images/2018-08-16-deploying-esxi-through-foreman/edit_host_uefi.png" width="400" />
  </p>

* **4.5:** Press 'Resolve' beside Provisioning templates and you should see the ESXi PXELinux and Kickstart templates.

  Note: The PXELinux template is irrelevant as it does not play any role in UEFI mode. So we are keeping it as is.

* **4.6:** Press 'Build', change the host's boot mode to UEFI and reboot the host to PXE. You should see the ESXi installer load up.

  Once the installation is complete and the host reboots, it will load mboot.efi again and the installation will restart. However since the host is not in build mode, it will not be able to fetch the kickstart file from Foreman and will throw an error and freeze at the installer screen. How do we overcome this?

  Well we already did! Due to the **before_provision** hook script we created above, after the installation is complete, the host's bootloader will be set to "None". So when the host boots it will try to load 'grub2/bootx64.efi' from the DHCP server. Since this bootloader does not exist (unless you have modified a default installation), it will boot into the next entry which ideally will be the ESXi boot medium.
